...
// ---------------------------------------------------------------------
// 3a. Honeypot check. The form includes a field named "website" that is
//     hidden from real users via CSS but visible to most naive bots,
//     which tend to fill in every field they find. A non-empty value
//     means this was almost certainly automated — we quietly pretend
//     success instead of telling the bot exactly why it failed.
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// 3b. Basic rate limiting. Restrict to one submission per 20 seconds per
//     session, to slow down scripted repeat submissions. This is a
//     lightweight deterrent, not a replacement for a proper WAF/captcha
//     if you find the form is being targeted heavily.
// ---------------------------------------------------------------------
$now             = time();
$lastSubmitAt    = $_SESSION['last_contact_submit'] ?? 0;
$minSecondsApart = 20;

if (($now - $lastSubmitAt) < $minSecondsApart) {
    respond(false, 'Please wait a few seconds before sending another message.', [], 429);
}

// ---------------------------------------------------------------------
// 4. Extract, trim, and validate input.
// ---------------------------------------------------------------------
...
try {
    ...
    // Record this submission time for rate limiting, now that we know
    // it was a genuine, successfully stored message.
